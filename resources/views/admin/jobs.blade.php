    ...
    <main class="flex-1 px-6 py-5">
            ...
            <div class="flex items-center gap-3 mb-1">
                ...
                <div>
                    ...
                    <p class="text-[12px] text-[#2d7f6a] font-semibold tracking-wide uppercase">
                        Monitoring seluruh lowongan JOBLYNX
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 py-4 flex justify-between items-center"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-white text-base">Daftar Lowongan</h3>
                            <p class="text-white/60 text-[11px]">Semua lowongan terdaftar di JOBLYNX</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-[13px]">
                        <thead>
                            <tr style="background-color: #f0faf7; border-bottom: 2px solid #c6ead9;">
                                ...
                                <th class="px-4 py-3.5 text-left text-[11px] font-extrabold uppercase tracking-wider text-[#2d7f6a]">Perusahaan</th>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[11px] text-gray-400 mt-0.5">ID #{{ $job->id }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                ...
                                <td class="px-4 py-4">
                                    @if($job->status_loker == 'Aktif')
                                        <span class="inline-flex items-center gap-1.5 bg-[#dcfce7] text-[#2d7f6a] px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-[#a7f3d0]">
                                            <span class="w-1.5 h-1.5 rounded-full bg-[#2d7f6a] inline-block"></span> AKTIF
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-gray-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-gray-400 inline-block"></span> TUTUP
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-2">
                                        <button
                                            onclick="openDetail('{{ $job->posisi }}','{{ $job->nama_perusahaan ?? '-' }}','{{ $job->status_loker }}','{{ $job->tipe_pekerjaan }}','{{ $job->gaji_min ?? 0 }}','{{ $job->gaji_max ?? 0 }}')"
                                        ...
                                        <button
                                            onclick="openDelete('{{ url('admin/jobs/delete/'.$job->id) }}')"
                            ...
                            <tr>
                                <td colspan="4" class="text-center py-20">
                                    <div class="inline-flex flex-col items-center gap-3">
                                        ...
                                        <span class="text-gray-400 text-sm font-medium">Belum ada lowongan tersedia</span>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
        ...
        <div id="detailModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-[999]">
            <div class="bg-white w-[430px] rounded-2xl shadow-xl p-6 relative">
                <button onclick="closeDetail()" class="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition">
                ...
                <div class="text-center mb-5">
                    ...
                    <h3 class="text-xl font-bold text-[#1a4450]">Detail Lowongan</h3>
                </div>
                <div class="space-y-4 text-sm">
                    <div><div class="text-gray-400 text-[11px] uppercase">Posisi</div><div id="detailPosisi" class="font-bold text-[#1a4450]"></div></div>
                    <div><div class="text-gray-400 text-[11px] uppercase">Perusahaan</div><div id="detailPerusahaan" class="font-medium text-gray-700"></div></div>
                    <div><div class="text-gray-400 text-[11px] uppercase">Status</div><div id="detailStatus" class="font-medium text-gray-700"></div></div>
                    <div><div class="text-gray-400 text-[11px] uppercase">Tipe Pekerjaan</div><div id="detailTipe" class="font-medium text-gray-700"></div></div>
                    <div><div class="text-gray-400 text-[11px] uppercase">Gaji</div><div id="detailGaji" class="font-medium text-gray-700"></div></div>
                </div>
            </div>
        </div>

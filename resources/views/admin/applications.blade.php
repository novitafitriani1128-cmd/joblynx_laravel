...
<head>
    <meta charset="UTF-8">
    <title>Admin Lamaran - JOBLYNX</title>
    ...
    <main class="flex-1 px-6 py-5">
            ...
            <div class="flex items-center gap-3 mb-1">
                ...
                <div>
                    ...
                    <p class="text-[12px] text-[#2d7f6a] font-semibold tracking-wide uppercase">
                        Monitoring seluruh data lamaran JOBLYNX
                    </p>
                </div>
            </div>
        </div>
                ...
                <div class="px-6 py-4 flex justify-between items-center"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center">
                            <i class="fa-solid fa-file-signature text-white text-base"></i>
                        </div>
                        <div>
                            <h3 class="font-bold text-white text-base">Data Lamaran</h3>
                            <p class="text-white/60 text-[11px]">Semua data lamaran yang masuk ke JOBLYNX</p>
                        </div>
                    </div>
                    <div class="bg-white/20 text-white text-xs font-bold px-3 py-1.5 rounded-full">
                        Total: {{ method_exists($applications, 'total') ? $applications->total() : count($applications) }} Lamaran
                    </div>
                </div>

                <!-- FILTER & SEARCH -->
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/80">
                    <form method="GET" action="{{ route('admin.applications') }}" class="flex flex-col sm:flex-row gap-3">
                        <!-- Search -->
                        ...
                        @if(request('search') || request('status'))
                            <a href="{{ route('admin.applications') }}" class="border border-gray-200 text-gray-500 px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-100 transition text-center">
                                Reset
                            </a>
                        @endif
                    </form>
                    ...
                    <table class="w-full text-[13px]">
                        <thead>
                            <tr style="background-color: #f0faf7; border-bottom: 2px solid #c6ead9;">
                                <th class="px-5 py-3.5 text-left text-[11px] font-extrabold uppercase tracking-wider text-[#2d7f6a]">Pelamar</th>
                                <th class="px-4 py-3.5 text-left text-[11px] font-extrabold uppercase tracking-wider text-[#2d7f6a]">Posisi</th>
                            ...
                            <tr class="hover:bg-[#f0faf7] transition-colors duration-150 group">
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-sm shrink-0
                                            bg-gradient-to-br from-[#2d7f6a] to-[#1a4450] text-white font-bold">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[11px] text-gray-400 mt-0.5">ID App #{{ $app->id }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-2 text-gray-600 text-sm">
                                        <i class="fa-solid fa-briefcase text-gray-300 text-xs"></i>
                                        {{ $app->posisi }}
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                    @if($app->status == 'Diterima')
                                        <span class="inline-flex items-center gap-1.5 bg-[#dcfce7] text-[#2d7f6a] px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-[#a7f3d0]">
                                            <span class="w-1.5 h-1.5 rounded-full bg-[#2d7f6a] inline-block"></span> DITERIMA
                                        </span>
                                    ...
                                    @elseif($app->status == 'Interview')
                                        <span class="inline-flex items-center gap-1.5 bg-blue-100 text-blue-500 px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-blue-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-blue-500 inline-block"></span> INTERVIEW
                                        </span>
                                    @elseif($app->status == 'Diproses')
                                        <span class="inline-flex items-center gap-1.5 bg-yellow-100 text-yellow-600 px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-yellow-200">
                                            <span class="w-1.5 h-1.5 rounded-full bg-yellow-500 inline-block"></span> DIPROSES
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full text-[10px] font-extrabold border border-gray-200">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button
                                            onclick="openDelete('{{ url('admin/applications/delete/' . $app->id) }}')"
                                ...
                                <td colspan="4" class="text-center py-20">
                                    <div class="inline-flex flex-col items-center gap-3">
                                        <div class="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center">
                                            <i class="fa-solid fa-file-signature text-3xl text-gray-300"></i>
                                        </div>
                                        <span class="text-gray-400 text-sm font-medium">Belum ada data lamaran</span>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/80 flex items-center justify-between">
                    <span class="text-xs text-gray-400">
                        @if(method_exists($applications, 'total'))
                            Menampilkan {{ $applications->firstItem() ?? 0 }}–{{ $applications->lastItem() ?? 0 }} dari {{ $applications->total() }} lamaran
                        @else
                            Total {{ count($applications) }} data lamaran terdaftar
                        @endif
                    </span>
                    @if(method_exists($applications, 'links'))
                        <div class="text-sm">{{ $applications->appends(request()->query())->links() }}</div>
                    @endif
                </div>
        ...
        <div id="detailModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-[999]">
            <div class="bg-white w-[420px] rounded-2xl shadow-xl p-6 relative">
                <button onclick="closeDetail()" class="absolute top-3 right-4 text-gray-400 hover:text-red-500">
                    <i class="fa-solid fa-xmark text-lg"></i>
                </button>
                <h3 class="text-xl font-bold text-[#1a4450] mb-5">Detail Lamaran</h3>
                <div class="space-y-4 text-sm">
                    <div>
                        <div class="text-gray-400 text-xs mb-1">Nama Pelamar</div>
                    ...
                    <div>
                        <div class="text-gray-400 text-xs mb-1">Posisi</div>
                        <div class="font-semibold text-[#1a4450]" id="detailPosisi"></div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="font-semibold text-[#1a4450]" id="detailTanggal"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="deleteModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-[999]">
            <div class="bg-white w-[360px] rounded-2xl shadow-xl p-6">
                ...
                <p class="text-sm text-gray-500 mb-5">Yakin ingin menghapus data lamaran ini?</p>
...
<script>
    ...
    // MODAL DETAIL
    function openDetail(nama, email, posisi, status, tanggal) {
